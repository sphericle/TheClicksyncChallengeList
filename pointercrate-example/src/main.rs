...
fn page_configuration() -> PageConfiguration {
    ...
    let nav_bar = NavigationBar::new("/static/images/path/to/your/logo.png")
        .with_item(
            ...
            html! {
                span style = "padding-left: 1.9rem; padding-right: 1.9rem" {
                    "List"
                }
            },
        )
    ...
    .with_column(FooterColumn::LinkList {
        heading: "Demonlist",
        links: vec![
            Link::new("/list/1/", "Current Top Demon"),
            Link::new(
                format!("/list/{}/", pointercrate_demonlist::config::list_size() + 1),
                "Extended List",
            ),
            Link::new(
                format!("/list/{}/", pointercrate_demonlist::config::extended_list_size() + 1),
                "Legacy List",
            ),
        ],
    })
