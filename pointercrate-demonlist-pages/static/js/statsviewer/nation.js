  ...
  onReceive(response) {
    ...
    formatInto(
      ...
      nationData.unbeaten.map((demon) =>
        this.formatDemon(demon, "/list/permalink/" + demon.id + "/")
      )
    );
    formatInto(
      this._beaten,
      ...
      nationData.verified.map((verification) => {
        return this.makeTooltip(
          this.formatDemon(
            ...
            "/list/permalink/" + verification.id + "/"
          ),
          "Verified&nbsp;by: ",
          verification.player
        );
